...
public class ManagerDatabase {
    ...
    public static ManagerDatabase getInstanceAfterKilledByOS(Context context) {
        instance = new ManagerDatabase(context);
        return instance;
    }
...
//                    FirebaseCrash.report(ex);
    ...
    public synchronized void createDb() {
        HelperALog.d(TAG, "createDb");
        // Create a new DB
        HelperDatabase dbHelper = new HelperDatabase(mCtx);
    ...
    public void exportDatabases() {
        ...
        if (files != null) {
            final String outFolder = new VaultFileUtil(mCtx).getFilePath("exportedDB");
            final File outFile = new File(outFolder);
            outFile.mkdirs();
            for (File file : files) {
                File tmp = new File(outFolder, file.getName());
                try {
                    CollageFilesUtils.copyFile(file, tmp);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
