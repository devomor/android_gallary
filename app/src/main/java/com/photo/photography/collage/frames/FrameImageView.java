...
public class FrameImageView extends ImageView {
    ...
    public FrameImageView(final Context context, PhotoItemCustom photoItem) {
        super(context);
...
//                    FirebaseCrash.report(err);
                ...
                HelperALog.d(TAG, "create FrameImageView, decode image");
            } else {
                HelperALog.d(TAG, "create FrameImageView, use decoded image");
            }
        }
    ...
    private static void setSpace(final float viewWidth, final float viewHeight, final PhotoItemCustom photoItem,
        ...
        if (photoItem.path != null) {
            ...
            if (photoItem.shrinkMethod == PhotoItemCustom.SHRINK_METHOD_3_3) {
            ...
            } else if (photoItem.shrinkMethod == PhotoItemCustom.SHRINK_METHOD_USING_MAP && photoItem.shrinkMap != null) {
                shrunkPoints = GeometryUtil.shrinkPathCollageUsingMap(convertedPoints, space, photoItem.shrinkMap);
            } else if (photoItem.shrinkMethod == PhotoItemCustom.SHRINK_METHOD_COMMON && photoItem.shrinkMap != null) {
    ...
    private static int findCenterPointIndex(PhotoItemCustom photoItem) {
    ...
    private static void buildRealPath(final float viewWidth, final float viewHeight,
                                      final PhotoItemCustom photoItem, final Path outPath,
        ...
        if (photoItem.path != null) {
            ...
            if (photoItem.cornerMethod == PhotoItemCustom.CORNER_METHOD_3_6) {
                outPath.computeBounds(bound, true);
                GeometryUtil.createRegularPolygonPath(outPath, Math.min(bound.width(), bound.height()), 6, corner);
                outPath.computeBounds(bound, true);
            } else if (photoItem.cornerMethod == PhotoItemCustom.CORNER_METHOD_3_13) {
                outPath.computeBounds(bound, true);
            ...
            float x = 0, y = 0;
            if (photoItem.shrinkMethod == PhotoItemCustom.SHRINK_METHOD_3_6 || photoItem.shrinkMethod == PhotoItemCustom.SHRINK_METHOD_3_8) {
    ...
    private static Path buildRealClearPath(final float viewWidth, final float viewHeight, final PhotoItemCustom photoItem, final Path clearPath, final float corner) {
        if (photoItem.clearPath != null) {
            ...
            float x = 0, y = 0;
            if (photoItem.shrinkMethod == PhotoItemCustom.SHRINK_METHOD_3_6) {
...
//            FirebaseCrash.report(ex);
    ...
    public boolean isSelected(float x, float y) {
        ...
        HelperALog.d(TAG, "isSelected, x=" + x + ", y=" + y + ", result=" + result);
        return result;
    }
