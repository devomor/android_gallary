 ...
 * expects.
 */
class ExpandableHListConnect extends BaseAdapter implements Filterable {
    /**
     ...
     * and group/child positions. See {@link ExpandableHListConnect.GroupMetadata}.
     */
     ...
     */
    public ExpandableHListConnect(ExpandableListAdapter expandableListAdapter) {
     ...
     * position encompassed in a {@link ExpandableHListConnect.PositionMetadata} object
    ...
    PositionMetadata getUnflattenedPos(final int flPos) {
        ...
        if (numExpGroups == 0) {
             ...
             * same as its flPos
             */
            return PositionMetadata.obtain(flPos, ExpandableHListPos.GROUP, flPos, -1, null, 0);
        }
        ...
        while (leftExpGroupIndex <= rightExpGroupIndex) {
            ...
            if (flPos > midExpGm.lastChildFlPos) {
                 ...
                 */
                return PositionMetadata.obtain(flPos, ExpandableHListPos.GROUP, midExpGm.gPos, -1, midExpGm, midExpGroupIndex);
                ...
                return PositionMetadata.obtain(flPos, ExpandableHListPos.CHILD, midExpGm.gPos, childPos, midExpGm, midExpGroupIndex);
            }
        }
        ...
        return PositionMetadata.obtain(flPos, ExpandableHListPos.GROUP, groupPos, -1, null, insertPosition);
    }
     ...
     * position.
     *
     * @param pos a {@link ExpandableHListPos} representing either a group position
     *            or child position
     * @return the flat list position encompassed in a {@link ExpandableHListConnect.PositionMetadata}
     * object that contains additional useful info for insertion, etc., or null.
     */
    PositionMetadata getFlattenedPos(final ExpandableHListPos pos) {
        final ArrayList<GroupMetadata> egml = mExpGroupMetadataList;
        final int numExpGroups = egml.size();
        ...
        while (leftExpGroupIndex <= rightExpGroupIndex) {
            ...
            if (pos.groupPos > midExpGm.gPos) {
                ...
                if (pos.type == ExpandableHListPos.GROUP) {
                ...
                } else if (pos.type == ExpandableHListPos.CHILD) {
    ...
    public boolean isEnabled(int flatListPos) {
        ...
        final ExpandableHListPos pos = metadata.position;

        boolean retValue;
        if (pos.type == ExpandableHListPos.CHILD) {
    ...
    public Object getItem(int flatListPos) {
        ...
        } else if (posMetadata.position.type == ExpandableHListPos.CHILD) {
    ...
    boolean collapseGroup(int groupPos) {
        ExpandableHListPos elGroupPos = ExpandableHListPos.obtain(ExpandableHListPos.GROUP, groupPos, -1, -1);
     ...
     * @see AdapterViews#findSyncPosition()
     */
    int findGroupPosition(long groupIdToMatch, int seedGroupPosition) {
        ...
        long endTime = SystemClock.uptimeMillis() + AdapterViews.SYNC_MAX_DURATION_MILLIS;
    ...
    static public class PositionMetadata {
        ...
        private static final ArrayList<PositionMetadata> sPool = new ArrayList<PositionMetadata>(MAX_POOL_SIZE);

        /**
         * Data type to hold the position and its type (child/group)
         */
        public ExpandableHListPos position;
        ...
        /**
         * Use {@link #obtain(int, int, int, int, ExpandableHListConnect.GroupMetadata, int)}
         */
        ...
        static PositionMetadata obtain(
            ...
            pm.position = ExpandableHListPos.obtain(type, groupPos, childPos, flatListPos);
