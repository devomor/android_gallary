...
class ExpandableHListPos {
    ...
    private static final ArrayList<ExpandableHListPos> sPool =
            new ArrayList<ExpandableHListPos>(MAX_POOL_SIZE);
    /**
    ...
    private ExpandableHListPos() {
    }

    static ExpandableHListPos obtainGroupPosition(int groupPosition) {
        return obtain(GROUP, groupPosition, 0, 0);
    }

    static ExpandableHListPos obtainChildPosition(int groupPosition, int childPosition) {
        return obtain(CHILD, groupPosition, childPosition, 0);
    }

    static ExpandableHListPos obtainPosition(long packedPosition) {
        if (packedPosition == ExpandableListView.PACKED_POSITION_VALUE_NULL) {
            return null;
        }

        ExpandableHListPos elp = getRecycledOrCreate();
    ...
    static ExpandableHListPos obtain(int type, int groupPos, int childPos, int flatListPos) {
        ExpandableHListPos elp = getRecycledOrCreate();
        elp.type = type;
    ...
    private static ExpandableHListPos getRecycledOrCreate() {
        ExpandableHListPos elp;
        synchronized (sPool) {
            if (sPool.size() > 0) {
                elp = sPool.remove(0);
            } else {
                return new ExpandableHListPos();
            }
        }
        elp.resetState();
        return elp;
