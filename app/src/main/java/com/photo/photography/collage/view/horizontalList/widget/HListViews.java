...
@RemoteView
public class HListViews extends AbsHListViews {
    ...
    public HListViews(Context context) {
        this(context, null);
    }

    public HListViews(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.hlv_listViewStyle);
    }

    public HListViews(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void addHeaderView(View v, Object data, boolean isSelectable) {
        ...
        if (mAdapter != null && !(mAdapter instanceof HeaderViewListRepeater)) {
    ...
    public boolean removeHeaderView(View v) {
        if (mHeaderViewInfos.size() > 0) {
            ...
            if (mAdapter != null
                    && ((HeaderViewListRepeater) mAdapter).removeHeader(v)) {
    ...
    public boolean removeFooterView(View v) {
        if (mFooterViewInfos.size() > 0) {
            boolean result = false;
            if (mAdapter != null
                    && ((HeaderViewListRepeater) mAdapter).removeFooter(v)) {
                if (mDataSetObserver != null) {
                    mDataSetObserver.onChanged();
                }
                result = true;
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
            mAdapter = new HeaderViewListRepeater(mHeaderViewInfos,
     ...
     *
     * @param childrenLeft  Left of the area in which children can be drawn, as measured
     *                      in pixels
     * @param childrenRight Right of the area in which children can be drawn, as measured
     *                      in pixels
    ...
    private void measureScrapChildWidth(View child, int position,
        ...
        if (p == null) {
            p = (AbsHListViews.LayoutParams) generateDefaultLayoutParams();
     ...
     * @param maxWidth                     The maximum height that will be returned (if all the children
    ...
    final int measureWidthOfChildren(int heightMeasureSpec, int startPosition,
        ...
        endPosition = (endPosition == NO_POSITION) ? adapter.getCount() - 1
                : endPosition;
        final AbsHListViews.RecycleBin recycleBin = mRecycler;
     ...
     * @param left     Pixel offset from the left of this view to the top of the
     *                 reference view.
     ...
     * @param flowDown    If true, align left edge to x. If false, align right edge to
     *                    x.
     * @param childrenTop Top edge where children should be positioned
     * @param selected    Is this position selected?
    ...
    private void setupChild(View child, int position, int x, boolean flowDown,
        ...
        AbsHListViews.LayoutParams p = (AbsHListViews.LayoutParams) child
                .getLayoutParams();
        if (p == null) {
            p = (AbsHListViews.LayoutParams) generateDefaultLayoutParams();
        }
        ...
        if ((recycled && !p.forceAdd)
                || (p.recycledHeaderFooter && p.viewType == AdapterViews.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
            attachViewToParent(child, flowDown ? -1 : 0, p);
        } else {
            p.forceAdd = false;
            if (p.viewType == AdapterViews.ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            if (recycled
                    && (((AbsHListViews.LayoutParams) child.getLayoutParams()).scrappedFromPosition) != position) {
     ...
     *
     * @param position Index (starting at 0) of the data item to be selected.
     * @param x        The distance from the left edge of the ListView (plus padding)
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            while (first.getRight() < listLeft) {
                AbsHListViews.LayoutParams layoutParams = (LayoutParams) first
                        .getLayoutParams();
            ...
            while (last.getLeft() > listRight) {
                AbsHListViews.LayoutParams layoutParams = (LayoutParams) last
                        .getLayoutParams();
                if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(HListViews.class.getName());
    }
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        info.setClassName(HListViews.class.getName());
    }
    ...
    static private class ArrowScrollFocusResult {
        ...
        /**
         * How
         * {@link HListViews#arrowScrollFocused}
         * returns its values.
         */
