...
import com.photo.photography.crop_image.anim.SimpleValueAnim;
import com.photo.photography.crop_image.anim.CallbackSimpleValueAnim;
import com.photo.photography.crop_image.anim.ValueAnimV14;
import com.photo.photography.crop_image.anim.ValueAnimV8;
import com.photo.photography.crop_image.listener.Callback;
...
public class CropImageViews extends AppCompatImageView {

    private static final String TAG = CropImageViews.class.getSimpleName();
    ...
    private SimpleValueAnim mAnimator = null;
    ...
    public CropImageViews(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void recalculateFrameRect(int durationMillis) {
        ...
        if (mIsAnimationEnabled) {
            SimpleValueAnim animator = getAnimator();
            animator.addAnimatorListener(new CallbackSimpleValueAnim() {
                @Override
    ...
    private SimpleValueAnim getAnimator() {
    ...
    private void setupAnimatorIfNeeded() {
        if (mAnimator == null) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                mAnimator = new ValueAnimV8(mInterpolator);
            } else {
                mAnimator = new ValueAnimV14(mInterpolator);
            }
        }
    }
    ...
    private Bitmap scaleBitmapIfNeeded(Bitmap cropped) {
        ...
        if (outWidth > 0 && outHeight > 0) {
            Bitmap scaled = Util.getScaledBitmap(cropped, outWidth, outHeight);
    ...
    private Uri saveImage(Bitmap bitmap, final Uri uri) throws IOException, IllegalStateException {
        ...
        try {
            ...
            Util.copyExifInfo(getContext(), mSourceUri, uri, bitmap.getWidth(), bitmap.getHeight());
            Util.updateGalleryInfo(getContext(), uri);
            return uri;
        } finally {
     ...
     * @see #loadAsync(Uri, LoadListener)
     */
    public void startLoad(final Uri sourceUri, final LoadListener callback) {
     ...
     * @param sourceUri Image Uri
     * @param callback  Callback
     * @see #loadAsync(Uri, boolean, RectF, LoadListener)
     */
    public void loadAsync(final Uri sourceUri, final LoadListener callback) {
    ...
    public void loadAsync(final Uri sourceUri, final boolean useThumbnail,
                          final RectF initialFrameRect, final LoadListener callback) {
     ...
     * @return Builder
     */
    public LoadRequests load(Uri sourceUri) {
        return new LoadRequests(this, sourceUri);
    }
    ...
    private Bitmap getImage(Uri sourceUri) {
        ...
        mExifRotation = Util.getExifOrientation(getContext(), mSourceUri);
        ...
        final Bitmap sampledBitmap =
                Util.decodeSampledBitmapFromUri(getContext(), mSourceUri, requestSize);
        mInputImageWidth = Util.sInputImageWidth;
        mInputImageHeight = Util.sInputImageHeight;
     ...
     * @see #save(Bitmap)
     */
    public void startCrop(final Uri saveUri, final CropListener cropCallback,
                          final SaveListener saveCallback) {
     ...
     */
    public void cropAsync(final Uri sourceUri, final CropListener cropCallback) {
        mExecutor.submit(new Runnable() {
            @Override
            public void run() {
                try {
    ...
    public void cropAsync(final CropListener cropCallback) {
     ...
     * @return Builder
     */
    public CropRequests crop(Uri sourceUri) {
        return new CropRequests(this, sourceUri);
    }
     ...
     * @param saveCallback Callback for saving the image
     */
    public void saveAsync(final Uri saveUri, final Bitmap image, final SaveListener saveCallback) {
        mExecutor.submit(new Runnable() {
     ...
     * @return Builder
     */
    public SaveRequests save(Bitmap bitmap) {
        return new SaveRequests(this, bitmap);
    }
