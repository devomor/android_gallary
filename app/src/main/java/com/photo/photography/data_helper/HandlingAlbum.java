...
public class HandlingAlbum extends SQLiteOpenHelper {
    ...
    private static HandlingAlbum mInstance = null;

    private HandlingAlbum(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
    }

    public static HandlingAlbum getInstance(Context context) {
        if (mInstance == null)
            mInstance = new HandlingAlbum(context);
        return mInstance;
    }
    ...
    public static ContentValues getDefaults(String path) {
        ...
        values.put(ALBUM_SORTING_MODE, SortingModes.DATE_DAY.getValue());
        values.put(ALBUM_SORTING_ORDER, SortingOrders.DESCENDING.getValue());
    ...
    @NonNull
    public static AlbumSetting getSettings(SQLiteDatabase db, String path) {
        Cursor cursor = null;
        try {
            if (exist(db, path)) {
                cursor = db.query(
            ...
            return AlbumSetting.getDefaults();
        } finally {
            if (cursor != null)
                cursor.close();
        }
    ...
    public ArrayList<String> getExcludedFolders(Context context) {
        ...
        HashSet<File> storageRoots = StorageHelpers.getStorageRoots(context);
