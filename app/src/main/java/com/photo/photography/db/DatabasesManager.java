...
public class DatabasesManager {
    ...
    private DatabasesManager(Context context) {
        if (context == null) {
    ...
    public static String getDbFileFullPath() {
        if (MyApp.getInstance() != null) {
            return getDbFileFullPath(MyApp.getInstance());
        } else {
    ...
    public static DatabasesManager getInstance() {
        if (instance == null) {
            instance = new DatabasesManager(MyApp.getInstance());
        }
        return instance;
    }

    public static DatabasesManager getInstance(Context context) {
        if (instance == null) {
            instance = new DatabasesManager(context);
        }
        return instance;
    }

    public static DatabasesManager getInstanceAfterKilledByOS(Context context) {
        instance = new DatabasesManager(context);
        return instance;
    }
...
//                    FirebaseCrash.report(ex);
    ...
    public synchronized void createDb() {
        AppLog.d(TAG, "createDb");
        // Create a new DB
        DatabaseHelpers dbHelper = new DatabaseHelpers(mCtx);
    ...
    public void exportDatabases() {
        ...
        if (files != null) {
            final String outFolder = new VaultFileUtil(mCtx).getFile("exportedDB").getAbsolutePath();
            final File outFile = new File(outFolder);
            outFile.mkdirs();
            for (File file : files) {
                File tmp = new File(outFolder, file.getName());
                try {
                    FileUtil.copyFile(file, tmp);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
