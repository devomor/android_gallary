...
public class ItemPackageTables extends BaseTables {
    ...
    public ItemPackageTables(Context context) {
        super(context);
    }
    ...
    public long insert(ItemPackageInfoModel info) {
    ...
    public int update(ItemPackageInfoModel info) {
        ContentValues values = new ContentValues();
        values.put(COLUMN_NAME, info.getTitle());
        values.put(COLUMN_THUMBNAIL, info.getThumbnail());
        values.put(COLUMN_SELECTED_THUMBNAIL, info.getSelectedThumbnail());
    ...
    public ItemPackageInfoModel getRowWithName(String name) {
        ...
        List<ItemPackageInfoModel> files = toList(filesCursor);
        filesCursor.close();
        if (files.size() > 0) {
            return files.get(0);
        } else {
    ...
    public ItemPackageInfoModel getRowWithStoreId(String idStr) {
        String[] selColumns = null;
    ...
    public List<ItemPackageInfoModel> getAllRows() {
        String[] selColumns = null;
    ...
    public List<ItemPackageInfoModel> getRows(String type) {
        String[] selColumns = null;
    ...
    private ItemPackageInfoModel cursorToItemPackage(Cursor itemsCursor) {
        ItemPackageInfoModel item = new ItemPackageInfoModel();
    ...
    private List<ItemPackageInfoModel> toList(Cursor itemsCursor) {
        List<ItemPackageInfoModel> itemList = new ArrayList<ItemPackageInfoModel>();
        if (itemsCursor.moveToFirst()) {
            do {
                ItemPackageInfoModel item = cursorToItemPackage(itemsCursor);
                itemList.add(item);
