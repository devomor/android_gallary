...
public class TextEntityHelper extends ImageEntityHelper {
    public static final Parcelable.Creator<TextEntityHelper> CREATOR = new Parcelable.Creator<TextEntityHelper>() {
        @Override
        public TextEntityHelper createFromParcel(Parcel in) {
            return new TextEntityHelper(in);
        }

        @Override
        public TextEntityHelper[] newArray(int size) {
            return new TextEntityHelper[size];
        }
    };
    ...
    public TextEntityHelper(String text, Resources res) {
        super(-1, res);
        mText = text;
        mTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,
                TextDrawableHelper.DEFAULT_TEXT_SIZE, res.getDisplayMetrics());
    }

    private TextEntityHelper(Parcel in) {
        super(in);
        mText = in.readString();
    ...
    protected Drawable createDrawableFromPrimaryInfo(Context context) {
        final TextDrawableHelper textDrawable = new TextDrawableHelper(mTextSize, mText);
    ...
    public void setText(String text) {
        mText = text;
        if (getDrawable() != null)
            ((TextDrawableHelper) getDrawable()).setText(mText);
    }

    public void setTextColor(int color) {
        mTextColor = color;
        if (getDrawable() != null)
            ((TextDrawableHelper) getDrawable()).setTextColor(mTextColor);
    }

    public void setTextSize(float textSize) {
        mTextSize = textSize;
        if (getDrawable() != null)
            ((TextDrawableHelper) getDrawable()).setTextSize(mTextSize);
    }

    public void setTypefacePath(String typefacePath) {
        mTypefacePath = typefacePath;
        if (getDrawable() != null)
            ((TextDrawableHelper) getDrawable()).setTypefacePath(mTypefacePath);
    }
    ...
    public void draw(Canvas canvas, float scale) {
        if (scale == 1) {
            ...
            canvas.save();
            final TextDrawableHelper textDrawable = (TextDrawableHelper) getDrawable();
    ...
    protected float[] calculateHalfDrawableSize(float scaleX, float scaleY) {
        TextDrawableHelper drawable = (TextDrawableHelper) getDrawable();
        if (mCurrentScale > 0) {
