...
import com.photo.photography.edit_views.DoubleClicksDetector;
import com.photo.photography.frames.TouchFrame;
...
import com.photo.photography.util.multi_touch.helper.MultiTouchHelper;
...
public class PhotosView extends View implements MultiTouchHelper.MultiTouchObjectCanvas<MultiTouchEntityHelper> {
    private static final long DOUBLE_CLICK_TIME_INTERVAL = 700;
    private static final int UI_MODE_ROTATE = 1, UI_MODE_ANISOTROPIC_SCALE = 2;
    private static final float SCREEN_MARGIN = 100;
    private final MultiTouchHelper<MultiTouchEntityHelper> multiTouchController = new MultiTouchHelper<MultiTouchEntityHelper>(
            this);
    private final MultiTouchHelper.PointInfo currTouchPoint = new MultiTouchHelper.PointInfo();
    private final boolean mShowDebugInfo = false;
    private final Paint mLinePaintTouchPointCircle = new Paint();
    // original code
    ...
    private MultiTouchEntityHelper mCurrentSelectedObject = null;
    private int mSelectedCount = 0;
    private long mSelectedTime = System.currentTimeMillis();
    private CallbackOnDoubleClick mClickListener = null;
    ...
    private MultiTouchEntityHelper mTouchedObject = null;
    private DoubleClicksDetector mDoubleClickDetector;
    ...
    private void init(Context context) {
        ...
        mDoubleClickDetector = new DoubleClicksDetector();
    ...
    private void cleanImages() {
        ...
        List<MultiTouchEntityHelper> entityList = new ArrayList<MultiTouchEntityHelper>();
        for (MultiTouchEntityHelper entity : mImages)
            if (!((ImageEntityHelper) entity).isNull()) {
    ...
    public void addImageEntity(MultiTouchEntityHelper entity) {
        if (mImages == null) {
            return;
        }

        if (mImages.size() > 0) {
            if ((mImages.get(0) instanceof ImageEntityHelper)
                    && (entity instanceof ImageEntityHelper)) {
                ImageEntityHelper img = (ImageEntityHelper) mImages.get(0);
                ((ImageEntityHelper) entity).setBorderColor(img.getBorderColor());
                ((ImageEntityHelper) entity).setDrawImageBorder(img
                        .isDrawImageBorder());
            }
        }
    ...
    public void setOnDoubleClickListener(CallbackOnDoubleClick listener) {
        this.mClickListener = listener;
    }

    public void setFrameTouchListener(CallbackOnFrameTouch frameTouchListener) {
    ...
    public void setBorderColor(int color) {
        ...
        int n = mImages.size();
        for (int i = 0; i < n; i++) {
            if (mImages.get(i) instanceof ImageEntityHelper) {
                ((ImageEntityHelper) mImages.get(i)).setBorderColor(color);
            }
        }
    ...
    public void setBorderSize(float borderSize) {
        ...
        for (int i = 0; i < n; i++) {
            if (mImages.get(i) instanceof ImageEntityHelper) {
                ((ImageEntityHelper) mImages.get(i)).setBorderSize(borderSize);
            }
        }
    ...
    public void setDrawImageBound(boolean drawImageBorder) {
        ...
        for (int i = 0; i < n; i++) {
            if (mImages.get(i) instanceof ImageEntityHelper) {
                ((ImageEntityHelper) mImages.get(i))
    ...
    public void setDrawShadow(boolean drawShadow) {
        ...
        for (int i = 0; i < n; i++) {
            if (mImages.get(i) instanceof ImageEntityHelper) {
                ((ImageEntityHelper) mImages.get(i)).setDrawShadow(drawShadow);
            }
        }
    ...
    public void setShadowSize(int shadowSize) {
        ...
        for (int i = 0; i < n; i++) {
            if (mImages.get(i) instanceof ImageEntityHelper) {
                ((ImageEntityHelper) mImages.get(i)).setShadowSize(shadowSize);
            }
        }
    ...
    public void setImageEntities(ArrayList<MultiTouchEntityHelper> images) {
        mImages = images;
    }
    ...
    public void setPhotoBackground(Uri photoBackgroundUri) {
        ...
        if (mPhotoBackgroundUri != null) {
            BitmapDrawable d = ImageDecoder.decodeUriToDrawable(getContext(), photoBackgroundUri);
    ...
    public Bitmap getImage(float outputScale) {
        ...
        int n = mImages.size();
        for (int i = 0; i < n; i++) {
            MultiTouchEntityHelper entity = mImages.get(i);
            if (entity instanceof ImageEntityHelper) {
                ((ImageEntityHelper) entity).draw(canvas, outputScale);
            } else {
                entity.draw(canvas);
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (mFrameTouchListener != null
                && mFrameTouchListener instanceof TouchFrame
                && ((TouchFrame) mFrameTouchListener).isImageFrameMoving()) {
     ...
     */
    @Override
    public MultiTouchEntityHelper getDraggableObjectAtPoint(MultiTouchHelper.PointInfo pt) {
        float x = pt.getX(), y = pt.getY();
        int n = mImages.size();
        for (int i = n - 1; i >= 0; i--) {
            ImageEntityHelper im = (ImageEntityHelper) mImages.get(i);
            if (im.contain(x, y))
                return im;
        }
        return null;
     ...
     */
    @Override
    public void selectObject(MultiTouchEntityHelper img, MultiTouchHelper.PointInfo touchPoint) {
     ...
     */
    @Override
    public void getPositionAndScale(MultiTouchEntityHelper img,
                                    MultiTouchHelper.PositionAndScale objPosAndScaleOut) {
     ...
     */
    @Override
    public boolean setPositionAndScale(MultiTouchEntityHelper img,
                                       MultiTouchHelper.PositionAndScale newImgPosAndScale, MultiTouchHelper.PointInfo touchPoint) {
        currTouchPoint.set(touchPoint);
        boolean ok = ((ImageEntityHelper) img).setPos(newImgPosAndScale);
        if (ok)
            invalidate();
        return ok;
    }
    ...
    @Override
    public boolean pointInObjectGrabArea(MultiTouchHelper.PointInfo pt, MultiTouchEntityHelper img) {
        return false;
    }
}
