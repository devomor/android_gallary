...
//    public static void addImageToGallery(final String filePath, final Context context, String folder) {
    ...
    public static void loadImageWithGlide(Context context, ImageView imageView, String uri) {
        if (uri != null && uri.length() > 1) {
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                Glide.with(context).load(uri)/*.diskCacheStrategy(DiskCacheStrategy.RESOURCE)*//*.crossFade()*/.into(imageView);
