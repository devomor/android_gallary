...
import com.photo.photography.models.FontItemModel;
...
public class TextUtil {
    ...
    public static List<FontItemModel> loadFonts(Context context) {
        final List<FontItemModel> result = new ArrayList<FontItemModel>();
        FontItemModel defaultFont = new FontItemModel(context.getString(R.string.default_font), null);
        ...
        try {
            ...
            if (fontNames != null) {
                for (String name : fontNames) {
                    FontItemModel item = new FontItemModel(name.substring(0, name.length() - 4),
                            PhotoUtil.ASSET_PREFIX.concat(FONT_FOLDER).concat("/").concat(name));
                    result.add(item);
                }
            }
        } catch (Exception ex) {
    ...
    public static Typeface loadTypeface(Context context, String path) {
        ...
        } else if (path.startsWith(PhotoUtil.ASSET_PREFIX)) {
            String assetPath = path.substring(PhotoUtil.ASSET_PREFIX.length());
