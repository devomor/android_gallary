...
public class FrameImageUtils {
    ...
    protected static TemplateItemModel collage(String frameName) {
        TemplateItemModel item = new TemplateItemModel();
        item.setPreview(PhotoUtil.ASSET_PREFIX.concat(FrameImageUtils.FRAME_FOLDER).concat("/").concat(frameName));
        item.setTitle(frameName);
        return item;
    }

    private static TemplateItemModel collage_1_0() {
        TemplateItemModel item = collage("collage_1_0.png");
    ...
    public static ArrayList<TemplateItemModel> loadFrameImages(Context context) {
        ...
        try {
            ...
            if (frameNames != null && frameNames.length > 0) {
                for (String str : frameNames) {
                    TemplateItemModel item = createTemplateItems(str);
                    if (item != null)
                        templateItemList.add(item);
                }

                Collections.sort(templateItemList, new Comparator<TemplateItemModel>() {
                    @Override
                    public int compare(TemplateItemModel lhs, TemplateItemModel rhs) {
    ...
    private static TemplateItemModel createTemplateItems(String frameName) {
